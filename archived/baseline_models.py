...
class MLBaseline:
    ...
    def _extract_features(self, person1_data: Dict, person2_data: Dict) -> np.ndarray:
        """
        Extract feature vector for a pair - FULL FEATURE PARITY WITH LLM.
        
        Now extracts ALL features that LLM sees in persona_narrative:
        - Demographics (age, gender, race, field, career)
        - Preferences (6 features)
        - Self-ratings (5 features)
        - Others' perception (5 features)
        - Interests with ratings (17 features)
        - Dating behavior (go_out, date, exphappy, goal)
        - Values (imprace, imprelig)
        - Derived features (overlap, alignment, age_diff, etc.)
        
        Total: ~90+ features (matching LLM information access)
        """
        features = []
        ...
        features.append(float(person1_data.get('age', 25)) / 100.0)  # Normalize
        features.append(float(person1_data.get('gender', 0)))  # 0=female, 1=male
        
        # Race (one-hot encode top 3: European=2, Asian=4, Latino=3)
        race1 = person1_data.get('race', 0)
        features.extend([
            1.0 if race1 == 2 else 0.0,
            1.0 if race1 == 4 else 0.0,
            1.0 if race1 == 3 else 0.0
        ])
        
        # Preferences (6 features)
        prefs1 = person1_data.get('preferences_self', {})
        features.extend([
        ...
        # Others' perception (5 features) - LLM sees this!
        ...
        # Interests (17 features with ratings) - LLM sees these!
        ...
        # Dating behavior (4 features) - LLM sees this!
        ...
        features.extend([
            1.0 if goal1 == 1 else 0.0,  # fun night
            1.0 if goal1 == 2 else 0.0,  # meet people
            1.0 if goal1 == 4 else 0.0   # serious relationship
        ])
        
        # Values (2 features) - LLM sees this!
        features.append(float(person1_data.get('imprace', 0) or 0))
        features.append(float(person1_data.get('imprelig', 0) or 0))
        
        # === PERSON 2 FEATURES (mirror of Person 1) ===
        ...
        # Preferences (6 features)
        prefs2 = person2_data.get('preferences_self', {})
        features.extend([
        ...
        # === DERIVED FEATURES (relationships between P1 and P2) ===
        
        # Interest overlap (Jaccard similarity with threshold > 5)
        ...
        # Preference-rating alignment (P1 values what P2 has)
        ...
        # Reverse alignment (P2 values what P1 has)
        ...
        age_diff = abs(age1 - age2) / 20.0  # Normalize
...
def evaluate_baselines(
    ...
    for pair in personas_pairs:
        if 'person1' in pair and 'person2' in pair:
            ...
            # Extract feature data for baseline models - USE PRE_EVENT_DATA
            # This ensures baselines have access to the SAME features as LLM
            person1_data = person1.get('pre_event_data', {})
            person2_data = person2.get('pre_event_data', {})
